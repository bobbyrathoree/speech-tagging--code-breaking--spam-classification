...
class Score:
    ...
    def score(self, algo_outputs, gt):
        ...
        for algo, labels in algo_outputs.items():
            ...
            self.sentence_scorecard[algo] = self.sentence_scorecard.get(algo, 0) + (
                correct == len(gt)
            )
    ...
    def print_scores(self):
        print(
            ...
            % (self.sentence_count, self.word_count)
        )
        ...
        for i in sorted(self.word_scorecard):
            print(
                ...
                % (
                    i,
                    self.word_scorecard[i] * 100 / float(self.word_count),
                    self.sentence_scorecard[i] * 100 / float(self.sentence_count),
                )
            )
    ...
    def print_helper(description, list, sentence):
        print(
            ...
            + " ".join(
                [
                    (("%-" + str(max(4, len(sentence[i]))) + "s") % list[i])
    ...
    def print_results(sentence, outputs, posteriors, models):
        Score.print_helper(
            " ".join([("%7s" % model) for model in models]), sentence, sentence
        )
        for algo in sorted(outputs.keys()):
            Score.print_helper(
                algo
                + "  "
                + " ".join(["%7.2f" % posteriors[algo][model] for model in models]),
                outputs[algo],
                sentence,
            )
